const Banner = ({ bannerImg, bannerTitle, titleSide, bannerMobile }) => {
  return (
    <>
      {/* desktop view */}
      <div className="relative hidden md:block">
        ...
        >
          <h2 className="text-5xl font-bold">{bannerTitle}</h2>
          ...
          <button className="uppercase bg-black/90 hover:bg-black/80 cursor-pointer text-white/90 text-sm font-bold px-14 py-[12px] mt-6">
            Shop Now
          </button>
        </div>
      </div>
      ...
      <div className="relative block md:hidden">
        ...
        <div className="absolute top-0 left-0 size-full bg-black/30 text-white flex flex-col justify-center items-center">
          <h2 className="text-2xl font-bold">{bannerTitle}</h2>
          <div className="border-2 w-14 border-black/90" />
          <p className="text-sm text-center w-[85%]">
            Quisquemos sodales suscipit tortor ditaemcos condimentum de cosmo
            lacus meleifend menean diverra loremous.
          </p>
          <button className="uppercase bg-black/90 text-white/90 text-sm font-bold px-14 py-[12px] mt-6">
            Shop Now
          </button>
        </div>
      </div>
